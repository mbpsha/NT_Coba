...
import OrderStatusAction from '@/Components/User/OrderStatusAction.vue'
  ...
  <div class="min-h-screen text-gray-900 bg-gray-100 font-inter">
    ...
    <main class="max-w-6xl px-4 pt-24 pb-16 mx-auto sm:px-6 lg:px-8">
      ...
      <div v-if="orders.length === 0" class="py-16 text-center bg-white rounded-lg shadow-sm">
        ...
        <button
          @click="router.visit('/toko')"
          class="px-6 py-2 mt-6 text-sm font-medium text-white bg-green-600 rounded-md hover:bg-green-700"
        >
          Mulai Belanja
        </button>
      </div>
            ...
            <div class="flex items-end justify-between pt-4 mt-4 border-t">
              <div class="space-y-1">
                ...
                <div v-if="order.shipping" class="pt-2 mt-2 text-xs text-gray-600 border-t">
                  ...
                  <p>
                    Ongkir:
                    <span class="font-semibold">{{ fmt(order.shipping.cost || 0) }}</span>
                  </p>

                  <p v-if="order.shipping.is_estimated" class="text-[11px] text-orange-600">
